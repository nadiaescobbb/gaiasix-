...
export function Spinner({ size = 'md', color = 'black' }) {
  const sizes = {
    sm: 'w-4 h-4 border-2',
    md: 'w-8 h-8 border-2',
    ...
    >
      {loading && (
        <div className="absolute inset-0 flex items-center justify-center bg-inherit rounded">
          <Spinner size="sm" color={className.includes('bg-white') ? 'black' : 'white'} />
        </div>
      )}
...
export function ProductCardWithLoading({ product, onAddToCart, imageLoading, setImageLoading }) {
  return (
